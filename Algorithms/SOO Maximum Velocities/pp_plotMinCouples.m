function pp_plotMinCouples(trajectories)
    ...
    hold on; grid on;
    plot(timeSteps,delta_s*ones(1,length(minDistances)),':','Color','k','LineWidth',1.2);
    xlabel('t [s]');
    ylabel("$d(t)\:[m]$",'Interpreter','latex')
    title({'Minimum distance between robots','with the Prioritized planning'});
    ylim([0 15])
    ...
    for i = 1:length(closestPairs)
        ...
        newPair = sprintf('R%d-%d', closestPairs{i}(1), closestPairs{i}(2));
