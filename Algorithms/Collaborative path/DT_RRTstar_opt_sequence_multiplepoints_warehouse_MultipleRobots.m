...
for loop = 1: Nrobots
    ...
    toc3=toc;
    total_time=toc1+toc2+toc3;
    
    
    %% Filling the "paths" variable with 1) n of map 2) sequence number 3) exec. time 4) path length
    if f==1
        paths_dtrrt(b,2)=f;
        paths_dtrrt(b,3)=toc3;
        paths_dtrrt(b,4)=total_distance;
    else
        paths_dtrrt((((f-1)*num_paths)+b),2)=f;
        paths_dtrrt((((f-1)*num_paths)+b),3)=toc3;
        paths_dtrrt((((f-1)*num_paths)+b),4)=total_distance;
    end
    %% Registering paths
    % if f==1
