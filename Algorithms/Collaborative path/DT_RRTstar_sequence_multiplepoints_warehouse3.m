...
OutputMatrix = cell(num_paths*num_of_sequences, maxLength+2);
...
%% Filling the OutputMatrix
if f==1
    numPoints = size(paths_registration{b}, 1);
    OutputMatrix{b, 1} = f; 
    OutputMatrix{b, 2} = total_distance;  
    
    for j = 1:numPoints
        OutputMatrix{b, 2 + j} = paths_registration{b}(j, :); % [x, y]
    end
else
    numPoints = size(paths_registration{((f-1)*num_paths)+b}, 1);
    OutputMatrix{((f-1)*num_paths)+b, 1} = f; 
    OutputMatrix{((f-1)*num_paths)+b, 2} = total_distance;
    
    for j = 1:numPoints
        OutputMatrix{((f-1)*num_paths)+b, 2 + j} = paths_registration{((f-1)*num_paths)+b}(j, :); % [x, y]
    end
end
maxLength = max(cellfun(@(x) size(x, 1), paths_registration));
for i = 1:((f-1)*num_paths)+b
    for j = size(paths_registration{i}, 1) + 3:maxLength+2
        OutputMatrix{i, j} = [Inf, Inf];
    end
end
...
OutputMatrix=OutputMatrix(:,1:maxLength+2);
...
% Iterate on each row of the OutputMatrix
for i = 1:numRows
...
excel_file = 'updatedMatrix_warehouse3.xlsx';
