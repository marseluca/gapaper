...
while num_points < num_couples
    x_start = rand * ((max(map.points(:,1))-safety_margin) - (min(map.points(:,1))+safety_margin)) + (min(map.points(:,1))+safety_margin);
    y_start = rand * ((max(map.points(:,2))-safety_margin) - (min(map.points(:,2))+safety_margin)) + (min(map.points(:,2))+safety_margin);
    x_goal = rand * ((max(map.points(:,1))-safety_margin) - (min(map.points(:,1))+safety_margin)) + (min(map.points(:,1))+safety_margin);
    y_goal = rand * ((max(map.points(:,2))-safety_margin) - (min(map.points(:,2))+safety_margin)) + (min(map.points(:,2))+safety_margin);
    ...
    if ~inside_any_polygon
        ...
        couples3(num_points, :) = [x_start, y_start, x_goal, y_goal];
    end
end