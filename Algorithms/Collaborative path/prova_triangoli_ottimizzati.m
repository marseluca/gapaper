...
for i = 1:length(paths_registration)
    % Ottieni la matrice corrente dalla cella
    current_path = paths_registration{i};
    
    % Estrai le coordinate x e y
    x = current_path(:, 1);
    y = current_path(:, 2);
    
    % Plotta i segmenti e i punti
    plot(x, y, '-o', 'MarkerFaceColor', 'r', 'MarkerEdgeColor', 'k');
    
    % Per un risultato più evidente, puoi usare differenti colori per ciascun percorso
    % Utilizzando la funzione 'plot' con differenti colori
