...
% video = VideoWriter('warehouse_fixed_200', 'MPEG-4');
% video.FrameRate = 1; 
% open(video); 
...
map.points=map.points*mapScaleFactor;
...
for i=1:size(mapp.lines,2)
    if mapp.points(1,mapp.lines(1,i))==0.02 && mapp.points(1,mapp.lines(2,i))==0.02
        ...
        for j=1:size(map.points,1)
            if x1*mapScaleFactor==map.points(j,1) && y1*mapScaleFactor==map.points(j,2) 
                jj=j;
            elseif x1*mapScaleFactor==map.points(j,2) && y1*mapScaleFactor==map.points(j,1)
                jj=j;
            end
        end
        for z=1:size(map.points,1)
            if x2*mapScaleFactor==map.points(z,1) && y2*mapScaleFactor==map.points(z,2) 
                zz=z;
            elseif x2*mapScaleFactor==map.points(z,2) && y2*mapScaleFactor==map.points(z,1)
                zz=z;
            end
        end
...
%%
visitedTrianglesSequences = {};
couples = startPoints;
for loopp = 1:size(couples,1)
    ...
    startNode=couples(loopp,:);
    % startNode=couples(loopp,1:2);
    % goalNode=couples(loopp,3:4);
    min_dist_triang=[];
    for i=1:size(triangulation.ConnectivityList,1)
    ...
    for f=1:num_of_sequences
        ...
        if nRowStart==nRowGoal
            ...
            for b=1:num_paths
                ...
                points_sequence=[startNode; goalNode];
                new_points_sequence=[startNode; goalNode];
            end
            break;
        end
        ...
        visitedTrianglesSequences{loopp} = visited_order;
        ...
        circle_radius = 0.003*mapScaleFactor; 
        ...
        for b=1:num_paths
            ...
            for i = 1:num_samples:size(samples.points, 1)-num_samples+1
                ...
                while contin
                    ...
                    contin = false;
                    % random_index = round(i+((num_samples-1)/2));
            ...
            new_points_sequence=adjustPath2(points_sequence, safety_margin, map);
            total_distance2=0;
            for i=1:size(new_points_sequence,1)-1
                total_distance2=total_distance2+sqrt((new_points_sequence(i,1) - new_points_sequence(i+1,1))^2 + (new_points_sequence(i,2) - new_points_sequence(i+1,2))^2);
            end
            % 
            % new_points_sequence2=adjustPath(new_points_sequence, safety_margin, map);
            % total_distance3=0;
            % for i=1:size(new_points_sequence2,1)-1
            %     total_distance3=total_distance3+sqrt((new_points_sequence2(i,1) - new_points_sequence2(i+1,1))^2 + (new_points_sequence2(i,2) - new_points_sequence2(i+1,2))^2);
            % end
            % 
            ...
            paths_dtrrt((((loopp-1)*num_paths)+b),2)=loopp;
            paths_dtrrt((((loopp-1)*num_paths)+b),3)=toc3;
            paths_dtrrt((((loopp-1)*num_paths)+b),4)=total_distance2;

            % end
            %% Registering paths
            paths_registration{((loopp-1)*num_paths)+b}=new_points_sequence;
...
fprintf("%d paths saved in paths_registration.mat",size(couples,1))
save("paths_registration.mat","paths_registration");

pathColors = distinguishable_colors(size(paths_registration,2));
save("pathColors.mat","pathColors");

pp_plotPathOnMap(paths_registration);

figure(1)
savefig("triangulated_warehouse.fig")