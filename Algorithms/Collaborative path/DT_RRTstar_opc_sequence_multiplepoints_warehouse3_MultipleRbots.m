...
% paths_registration = cell((num_paths*num_of_sequences*size(couples3,1)), 1);
paths_registration = cell((num_paths*num_of_sequences), 1);

% paths_dtrrt_3 = inf(num_paths*num_of_sequences*size(couples3,1), 4);
paths_dtrrt_3 = inf(num_paths*num_of_sequences, 4);
paths_dtrrt_3(:,1)=1;


maxLength = (max(cellfun(@(x) size(x, 1), paths_registration)));
% dynMatrix = cell(num_paths*num_of_sequences*size(couples3,1), maxLength+2);
...
for loopp = 1: 1                %size(couples3,1)
    ...
    % startNode = [robot_x_coords(loopp), robot_y_coords(loopp)];
    ...
    for f=1:num_of_sequences
        ...
        if nRowStart==nRowGoal
            ...
            toc3=toc;
            for b=1:num_paths
                paths_dtrrt_3((((loopp-1)*num_paths)+b),2)=loopp;
                paths_dtrrt_3((((loopp-1)*num_paths)+b),3)=toc3;
                paths_dtrrt_3((((loopp-1)*num_paths)+b),4)=total_distance;
            end
            break;
        end
        ...
        while ~isempty(queue)
                ...
                adj_to_goal=find(n_triangle_==nRowGoal);

                if size(adj_to_goal,1)==1
                    disp('///////////////////////////////////////////////////////////////////////////////////');
                    loop=true;
                    bueno=true;
                    adjacentTriangles = [adjacentTriangles; nRowGoal];
                    visited_order=[visited_order; nRowGoal];
                end
        ...
        for b=1:num_paths
            ...
            % if loopp==1
            %     paths_dtrrt(loopp*b,2)=loopp;
            %     paths_dtrrt(loopp*b,3)=toc3;
            %     paths_dtrrt(loopp*b,4)=total_distance;
            % else
