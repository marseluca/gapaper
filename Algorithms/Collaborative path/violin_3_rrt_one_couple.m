...
load('Workspaces/paths_rrt_3_one_couple.mat');
% load('Workspaces/paths_rrt_3_one_couple_2.mat');

%% RRT*
v1=violin({paths_3(:,2)}, 'facecolor', [0.8 0.2 0.2], 'edgecolor', [0.8 0.2 0.2]);

ax = gca; 
ax.XTickLabel = [];

xlabel('Map2,RRT*,first fixed couple');
% xlabel('Map2,RRT*,second fixed couple');
ylabel('Time [s]');
...
means = cellfun(@mean, {paths_3(:,2)});
medians = cellfun(@median, {paths_3(:,2)});
