...
for iter = 1:maxIter
    ...
    [middle_done,tree, nearNodes, newNode, rewiredNodes, nearestNode] = extendTree_triangulation(middle_done, middle_points, max_x, min_x, max_y, min_y, tree, rewiringRadius, map, triangulation); %create q_new nodo (con coordinate)
    
    if distance(tree.nodes(end).position, goalNode) < goalThreshold
        goalNode_ = tree.nodes(end);
...
% coll=false;
% 
% s_=[s_; triangulation.ConnectivityList(1,1) triangulation.ConnectivityList(1,2); triangulation.ConnectivityList(1,2) triangulation.ConnectivityList(1,3)];
% 
% p1=triangulation.Points(s_(1,1),:);
% p2=triangulation.Points(s_(1,2),:);
% p3=triangulation.Points(s_(2,1),:);
% p4=triangulation.Points(s_(2,2),:);
% 
% p5=triangulation.Points(triangulation.ConnectivityList(16,2),:);
% p6=triangulation.Points(triangulation.ConnectivityList(16,3),:);
% 
% 
% if ~segmentsIntersect(p5,p6,middle_points(1,:),middle_points(7,:))
%     coll=true;
% end
