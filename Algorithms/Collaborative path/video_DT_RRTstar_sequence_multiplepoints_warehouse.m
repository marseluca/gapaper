...
paths = inf(num_paths*num_of_sequences, 4);
paths(:,1)=1;
...
%% Iterate for the number of triangle sequences you want to obtain (in this case 2)
...
for b=1:num_paths
    ...
    % if f==1
    %     if b==1
    %         total_distance11=total_distance;
    %     elseif b==2
    %         total_distance21=total_distance;
    %     elseif b==3
    %         total_distance31=total_distance;
    %     end
    % elseif f==2
    %     if b==1
    %         total_distance12=total_distance;
    %     elseif b==2
    %         total_distance22=total_distance;
    %     elseif b==3
    %         total_distance32=total_distance;
    %     end
    % elseif f==3
    %     if b==1
    %         total_distance13=total_distance;
    %     elseif b==2
    %         total_distance23=total_distance;
    %     elseif b==3
    %         total_distance33=total_distance;
    %     end
    % end
    tic;
    ...
    hold on;
    %% To plot the paths in different colours
    % if f==1
    %     if b==1 && stop==false
...
%% Filling the "paths" variable with 1) n of map 2) sequence number 3) exec. time 4) path length
if f==1
    paths(b,2)=f;
    % paths(b,3)=total_time;
    paths(b,4)=total_distance;
else
    paths((((f-1)*num_paths)+b),2)=f;
    % paths((((f-1)*num_paths)+b),3)=total_time;
    paths((((f-1)*num_paths)+b),4)=total_distance;
end
%% Registering paths
if f==1
    paths_registration{b}=points_sequence;
else
    paths_registration{((f-1)*num_paths)+b}=points_sequence;
end
...
% updatedMatrix=cell(size(dynMatrix,1),size(dynMatrix,2));
% oldDyn=dynMatrix;
...
%         rowCoords_ = dynMatrix(i, 3:end);
%         nonEmptyIndices = ~cellfun(@isempty,  rowCoords_);
% 
%         % Seleziona solo gli elementi non vuoti
%         rowCoords = rowCoords_(nonEmptyIndices);
% 
...
%             newPoints = points;
%             j = 1;
%             while j < size(points, 1) && cont_inf~=0
...
%                 cont_inf=cont_inf-1;
%             end
%             newRowCoords = cell(1, size(newPoints, 1));
%             for z = 1:size(newPoints, 1)
%                 newRowCoords{z} = newPoints(z, :);
%             end
%             updatedMatrix(i, 1)=dynMatrix(i, 1);
%             updatedMatrix(i, 2)=dynMatrix(i, 2);
%             updatedMatrix(i, 3:2+length(newRowCoords)) = newRowCoords;
%         else
%             updatedMatrix(i, :)=dynMatrix(i, :);
%         end
% 
